...
def get_id_to_tiles_from_geometry(stream_files: list[str], output_json: str):
    id_to_tile = defaultdict(list)

    with Pool(processes=20) as pool:
        for local_map in tqdm.tqdm(
            pool.imap_unordered(process_stream_file, stream_files),
            total=len(stream_files),
            desc="Processing stream files",
        ):
...
def make_floodmap_helper(args):
    return make_floodmap(*args)


def run_c2f(flow_files: list[str]):
    args_list = []
...
if __name__ == "__main__":
    ...
    # with tqdm.tqdm(total=100, desc="Building final GeoTIFF") as pbar:
    #     def callback(complete, message, data):
    #         pbar.update(int(complete * 100 - pbar.n))

    #     options = gdal.WarpOptions(format='COG', 
    #                                warpMemoryLimit=1024*100,
    ...
    #     gdal.Warp(r"C:\Users\lrr43\Downloads\test_majority.tif", 'temp.vrt', options=options)
